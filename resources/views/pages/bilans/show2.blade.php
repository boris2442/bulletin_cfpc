...
<div class="grid grid-cols-2 gap-4 mb-6 text-sm uppercase">
    <div class="space-y-1">
        <p>Nom / Name: <span class="font-bold">{{ $etudiant->name }}</span></p>
        {{-- Ajout de la Classe/Niveau --}}
        <p>Classe / Level: <span class="font-bold">{{ $etudiant->inscriptions->first()->classe->nom_classe ?? 'N/A' }}</span></p>
        <p>Spécialité: <span class="font-bold">{{ $etudiant->inscriptions->first()->specialite->nom_specialite ?? 'N/A' }}</span></p>
    </div>
    <div class="space-y-1 text-right">
        <p>Année Académique: <span class="font-bold">{{ $anneeActive->date_debut->format('Y') }}-{{ $anneeActive->date_fin->format('Y') }}</span></p>
        <p>ID Étudiant: <span class="font-bold text-lg text-blue-700">{{ str_pad($etudiant->id, 5, '0', STR_PAD_LEFT) }}</span></p>
    </div>
</div>
